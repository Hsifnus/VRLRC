...
public class PuzzleManager : MonoBehaviour
{
    ...
    public class PuzzleCondition
    {
        ...
        public PuzzleCondition(PuzzleManager manager, string condition)
        {
        ...
        private float GetActivation()
        {
            ...
            {
                return activator.GetComponent<LeverState>().GetActivation();
            } else if (activatorType == "Pressure Plate")
            {
                return activator.GetComponent<PressurePlateState>().GetActivation();
            }
        ...
        public void UpdateState()
        {
            for (int i = 0; i < activatees.Count; i++)
            {
                ...
                {
                    activatees[i].GetComponent<PlatformMovement>().SetState(activateeStates[i]);
                }
                else if (activateeTypes[i] == "Teleporter")
                {
                    activatees[i].GetComponent<Teleporter>().SetState(activateeStates[i]);
                } else
                {
    ...
    void Start()
    {
        ...
        if (levers)
        {
            Debug.Log("levers childCount: " + levers.transform.childCount);
        ...
        if (plates)
        {
            Debug.Log("plates childCount: " + plates.transform.childCount);
    ...
    {
        dirty = true;
        Debug.Log("Puzzle manager update requested.");
    }
