...
public class PlayerLocomotion_Client : Photon.PunBehaviour, IPunCallbacks {
    ...
    override public void OnPhotonInstantiate (PhotonMessageInfo info) {
        Debug.Log("OnPhotonInstantiate 4");
        if (isLeft)
        {
            eye = controller = GameObject.FindGameObjectsWithTag("BaseControllerLeft")[0];
        } else
        {
